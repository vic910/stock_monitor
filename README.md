...
| 实时做T | 盘中高抛低吸三态信号：低吸（红）/持有（灰）/高抛（绿），以当日均价为中枢，悬停单元格看参考价与依据（见下节） |
...
| 实时做T | 盘中买卖点三态：低吸/持有/高抛，均价中枢+区间位置+量能过滤（见下节） |
...
给盘中高抛低吸的三态信号，数据全部来自实时行情接口（今开/昨收/当日高低/成交额量），**仅交易时段（9:30–11:30 / 13:00–15:00）计算**，非时段或无数据显示 `--`。

以**当日均价**（成交额 ÷ 成交量）为中枢，用偏离度 `dev` 与日内区间位置 `pos` 定位，再叠加量能、日线趋势过滤：

| 显示 | 条件（同时满足） | 颜色 |
|---|---|---|
| 低吸 | 回踩到均价下方(dev ≤ −k)或日内低位(pos ≤ 20%)、已从当日最低回升（不接飞刀）、且未破位放量下杀 | 红 |
| 高抛 | 冲高到均价上方(dev ≥ +k)或日内高位(pos ≥ 80%)、且已冲高回落（滞涨） | 绿 |
| 持有 | 未触发上述任一 | 灰 |

- 阈值 `k` 按**当日振幅自适应**（振幅大→k大，范围 ±0.5%~±3%），不搞一刀切。
- 破位放量（pos ≤ 10% 且量比 ≥ 2）判为下跌加速，**否决低吸**，避免抄在半山腰。
- **鼠标悬停单元格**可看完整依据：现价/均价/dev、日内位置/振幅/阈值k、量比、日线多空、参考买入价与高抛目标价，以及信号强度（缩量+趋势配合越多，`●` 越多、越可信）。
