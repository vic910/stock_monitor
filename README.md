...
- **收益排行**：K线窗口底部「收益排行」按钮、以及策略回测表格「操作」列的「排行」按钮均可打开。排行按 **作用域=股票+时间段** 分榜，同一股票同时间段下不同买卖条件按 **收益率% 降序**；同一(买入,卖出)描述去重更新。榜单**持久化到本地**（`config.rank_store`），重启后仍可查看。点任意行**在后台重算**该组合并打开K线窗口（同时只保留一个K线窗口）。
