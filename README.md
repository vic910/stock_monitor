...
| 排行辅助 | 排行窗口支持：**区间买入持有**收益对照、**删除单个作用域**、给作用域加**备注**（下拉标签同步显示）、**一键截图**整窗保存到配置目录 |
...
│   └── _open_leaderboard(code)/_open_kline()  # 打开排行窗（可按 code 过滤下拉）/ 唯一非模态K线窗
├── RankDialog                 # 收益排行窗口：作用域下拉 + 收益率降序表格 + 组合排行/备注/删除/截图；点行后台重算开K线窗
